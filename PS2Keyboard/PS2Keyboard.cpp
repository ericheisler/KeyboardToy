...
void PS2Keyboard::interruptable(bool able){
	if(able){
		// wait till there is no incoming
		now_ms = millis();
		prev_ms = now_ms;
		while(now_ms - prev_ms < 50){
			now_ms = millis();
...
void PS2Keyboard::begin(uint8_t data_pin, uint8_t irq_pin1, const PS2Keymap_t &map) {
  irq_num=255;
  irq_pin = irq_pin1;
